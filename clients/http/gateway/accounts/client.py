...
class OpenDebitCardAccountRequestDict(TypedDict):
    """
    Структура данных для открытия счета дебетовой карты.
    """
    userId: str
...
class OpenCreditCardAccountRequestDict(TypedDict):
    """
    Структура данных для открытия счета кредитной карты.
    """
    userId: str
...
class AccountsGatewayHTTPClient(HTTPClient):
    ...
    def get_accounts_api(self, query: GetAccountsQueryDict) -> Response:
        ...
        :param query: Словарь параметров запроса, например: {'userId': '123'}
